...
class App {
    void run() {
        List<Inventory> inventotyList = new ArrayList<>();
        int number = 0;
        ...
        while (true) {
            ...
            if (cmd.equals("종료")) { //참이 면 실행
                ...
                inventotyList.add(inventory); // 객체를 리스트에 저장한다.
            } else if (cmd.equals("목록")) { //참이면 실행

                System.out.println("번호 / 작가 / 명언");
                System.out.println("-------------------");
                for (int i = inventotyList.size() - 1; i >= 0; i--) { // 마지막에 입력했던 인덱스 를 선수로 출력
                    Inventory inventory = inventotyList.get(i);
